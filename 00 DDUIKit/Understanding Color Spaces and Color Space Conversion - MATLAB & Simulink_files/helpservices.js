   ...
   $(window).bind("click","a",function(evt) {
       if (evt.target) {
           ...
           if (href && (! href.match(/javascript:/))) {
               ...
               } else if (!_isLocalConnectorUrl(pageUrl) 
                    && pageUrl.indexOf("mathworks.com/help") > 0 
                    && href.indexOf("mathworks.com/help") < 0
                    && (!_isLocalConnectorUrl(href))) {
...
function getMatlabCommand(href) {
    ...
    var match = isMatlabColonLink(href);
    if (match) {
...
function handleUninstalledDocLink(evt) {
    var href = $(this).attr("href");
    if (href && (! href.match(/javascript:/i)) && !isMatlabColonLink(href) && !_isMiddleMouse(evt) && !evt.ctrlKey && href.indexOf('#') !== 0 && evt.target.target !== '_blank') {
        evt.preventDefault();
        var aTag = this;
...
function handleMatlabOnlineDocLinksClick() {
    $(window).bind("click","a",function(evt) {
        if (evt.target) {
            var href = getSelectedUrl(evt.target);
            if (href) {
                if (!isMatlabColonLink(href) && ('_blank' !== evt.target.target)) {
