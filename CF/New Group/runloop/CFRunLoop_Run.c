...
static int32_t __CFRunLoopRun(CFRunLoopRef rl,
    ...
    //create main queue send & receive port
    
//
//#if USE_DISPATCH_SOURCE_FOR_TIMERS  macos only
...
//#if USE_DISPATCH_SOURCE_FOR_TIMERS。macOS
...
//#endif
    ...
    handle_msg:;
        ...
        if (MACH_PORT_NULL == livePort) {
            CFRUNLOOP_WAKEUP_FOR_NOTHING();//没有事件的唤醒
        } else if (livePort == rl->_wakeUpPort) {
            CFRUNLOOP_WAKEUP_FOR_WAKEUP();  //就是当前活动的mach端口
        }
//#if USE_DISPATCH_SOURCE_FOR_TIMERS   //macOS
...
//#endif
...
#endif
        else if (livePort == dispatchPort) {//x主线程事件
        ...
        if (sourceHandledThisLoop && stopAfterHandle) {
            ...
            retVal = kCFRunLoopRunTimedOut;//用runloop做超时监控
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl,
    ...
    __CFRunLoopLock(rl);
    CFRunLoopModeRef currentMode = __CFRunLoopFindMode(rl, modeName, false);/* call with rl locked, returns mode locked */
    ...
    if (currentMode->_observerMask & kCFRunLoopEntry )
        __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry);   ////1
    result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode);                                       /////////2
    if (currentMode->_observerMask & kCFRunLoopExit )
        __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopExit);    //3
